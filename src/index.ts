...
const verifyToken = (token: string, jwk: string) => {
  const publicKey = crypto.createPublicKey({ key: jwk, format: 'jwk' }).export({ format: 'pem', type: 'spki' })
...
app.get('/oidc/callback', async (req, res) => {
  ...
  try {
    const tokenResponse = await fetch('http://localhost:3000/openid-connect/token', {
      method: 'POST',
      headers: {
    ...
    const configuration = await (await fetch('http://localhost:3000/openid-connect/.well-known/openid-configuration')).json();
    const jwksUri = configuration['jwks_uri']
    const jwks = await (await fetch(jwksUri)).json();
    const jwk = jwks.keys.find((jwk: any) => jwk.kty === 'RSA' && jwk.alg === 'RS256' && jwk.use === 'sig');
